  ...
  //m_reco_pion_candidate_idxs_sideband()
{
  ...
  m_w_type    = is_mc ? GetWSidebandType(*universe, signal_definition, sidebands::kNWFitCategories) :
                        kNWSidebandTypes;
}
...
RecoPionIdx GetHighestEnergyPionCandidateIndex(const CCPiEvent& e) {
  return e.m_universe->GetHighestEnergyPionCandidateIndex(e.m_reco_pion_candidate_idxs);
}
...
void ccpi_event::FillRecoEvent(const CCPiEvent& event,
  ...
  if (event.m_is_mc && event.m_is_signal && event.m_passes_cuts) {
    ...
    if (HasVar(variables, "thetamu_deg") && HasVar(variables, "thetamu_deg_true"))
...
void ccpi_event::FillWSideband(const CCPiEvent& event,
  ...
  for(auto var : variables) {
    ...
    if (event.m_is_mc) {
      switch(event.m_w_type) {
        case kWSideband_Signal: 
          var->m_hists.m_wsidebandfit_sig.FillUniverse(*event.m_universe,
                                                       fill_val,
                                                       event.m_weight);
          break;
        case kWSideband_Low:
          var->m_hists.m_wsidebandfit_loW.FillUniverse(*event.m_universe,
                                                       fill_val,
                                                       event.m_weight);
          break;
        case kWSideband_Mid:
          var->m_hists.m_wsidebandfit_midW.FillUniverse(*event.m_universe,
                                                        fill_val,
                                                        event.m_weight);
          break;
        case kWSideband_High:
          var->m_hists.m_wsidebandfit_hiW.FillUniverse(*event.m_universe,
                                                       fill_val,
                                                       event.m_weight);
          break;
        default:
...
void ccpi_event::FillEfficiencyDenominator(const CCPiEvent& event,
    const std::vector<Variable*>& variables) {
  for (auto var : variables) {
...
void ccpi_event::FillCounters(const CCPiEvent& event, 
  ...
  for (auto i_cut : kCutsVector) {
    if (event.m_is_truth != IsPrecut(i_cut)) continue; // truth loop does precuts
...
void ccpi_event::FillCutVars(CCPiEvent& event, 
  ...
  for (unsigned int i = 0; i < kCutsVector.size(); ++i) {
    ...
    event.m_reco_pion_candidate_idxs.clear();
    pass = pass && PassesCut(*universe, cut, is_mc, sd,
                             endpoint_michels, vertex_mich);
    if (!pass) continue;
    ...
    if (next_cut == kAtLeastOneMichel && HasVar(variables, "michel_count")) {
      ...
      FillStackedHists(event, GetVar(variables, "michel_count"),
                       fill_val);
...
void ccpi_event::FillStackedHists(const CCPiEvent& event, Variable* v,
  ...
  v->GetStackComponentHist(GetSignalBackgroundType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );

  v->GetStackComponentHist(GetWSidebandType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );

  v->GetStackComponentHist(GetMesonBackgroundType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );

  v->GetStackComponentHist(GetWBackgroundType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );

  v->GetStackComponentHist(GetTruthWType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );

  v->GetStackComponentHist(GetCoherentType(*event.m_universe,
      event.m_signal_definition))->Fill( fill_val, event.m_weight );
}
