...
class EventSelectionPlotInfo {
  public: 
    ...
    EventSelectionPlotInfo(Variable* variable, 
                           float mc_pot, float data_pot,
                           bool do_frac_unc, bool do_cov_area_norm, bool include_stat,
    ...
    EventSelectionPlotInfo(float mc_pot, float data_pot,
                           bool do_frac_unc, bool do_cov_area_norm, bool include_stat,
                           SignalDefinition signal_definition)
    :
      m_mnv_plotter(kCCNuPionIncStyle), 
    ...
    void SetXLabel(PlotUtils::MnvH1D* hist) {
      std::string label = m_variable->m_hists.m_xlabel + " (" + m_variable->m_units + ")";
      if (hist)
...
//==============================================================================
  ...
  void Plot_ErrorGroup(EventSelectionPlotInfo p, PlotUtils::MnvH1D* h,
    ...
    if (error_group_name=="LEGENDONLY") {
      ...
      p.m_mnv_plotter.DrawErrorSummary(hist, legend_position, p.m_include_stat,
                                       true, ignore_threshold,
                                       p.m_do_cov_area_norm, error_group_name,
                                       p.m_do_frac_unc);
    }
...
//==============================================================================
  void PlotVar_Selection(EventSelectionPlotInfo p, double ymax = -1.,
                         ...
                         bool do_tuned_bg = false, bool do_bin_width_norm = true) {
    ...
    PlotUtils::MnvH1D* mc = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_selection_mc.hist->Clone("mc");

    // Background
    PlotUtils::MnvH1D* tmp_bg = nullptr;
    if (do_bg) {
      if (do_tuned_bg)
        tmp_bg = (PlotUtils::MnvH1D*)(p.m_variable->m_hists.m_tuned_bg)->Clone("bg_tmp");
      else
        tmp_bg = (PlotUtils::MnvH1D*)(p.m_variable->m_hists.m_bg.hist)->Clone("bg_tmp");
    }
    else
      tmp_bg = NULL;
    ...
    std::string outfile_name = Form("Selection_%s_%s_%s%s%s%s", 
                                    p.m_variable->Name().c_str(), 
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(), 
                                    logy_str.c_str(), bg_str.c_str(),
                                    bwn_str.c_str());
    p.m_mnv_plotter.MultiPrint(&canvas, outfile_name, "png");
  }
  ...
  void PlotVar_ErrorSummary(EventSelectionPlotInfo p) {
    ...
    PlotUtils::MnvH1D* sel = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_selection_mc.hist->Clone(uniq());
...
//==============================================================================
  void Plot_BGSub(EventSelectionPlotInfo p, 
                  std::string outdir = ".", double ymax = -1,
                  bool do_log_scale = false, bool do_bin_width_norm = true) {
    std::cout << "Plotting BG-subtracted Data " << p.m_variable->Name()<< std::endl;
    ...
    // Get Hists
    TH1D* bg_sub_data_w_tot_error  = new TH1D(p.m_variable->m_hists.m_bg_subbed_data->GetCVHistoWithError());
    TH1D* bg_sub_data_w_stat_error = new TH1D(p.m_variable->m_hists.m_bg_subbed_data->GetCVHistoWithStatError());
    TH1D* effnum_w_stat_error      = new TH1D(p.m_variable->m_hists.m_effnum.hist->GetCVHistoWithStatError());
    ...
    // Draw
    const bool use_hist_titles = false;
    p.m_mnv_plotter.DrawDataMCWithErrorBand(bg_sub_data_w_tot_error, effnum_w_stat_error, pot_scale, "TR", use_hist_titles);

    //p.m_mnv_plotter.DrawDataMCWithErrorBand(bg_sub_data_w_stat_error, effnum_w_stat_error, pot_scale, "TR");
    //p.m_mnv_plotter.DrawDataMC(bg_sub_data_w_tot_error, effnum_w_stat_error, pot_scale, "TR");
    //p.m_mnv_plotter.DrawDataMCWithErrorBand(bg_sub_data_w_error, effnum, pot_scale, "TR",
    //                                        use_hist_titles, NULL, NULL, p.m_do_cov_area_norm, p.m_include_stat);
    ...
    std::string outfile_name = Form("%s/BGSubData_%s_%s_%s%s%s", 
                                    outdir.c_str(),
                                    p.m_variable->Name().c_str(), 
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(), 
  ...
  void PlotBGSub_ErrorGroup(EventSelectionPlotInfo p, std::string error_group_name, 
                            double ignore_threshold = 0., double ymax = -1.) {
    TCanvas canvas ("c1","c1"); 
    ...
    // Get the histo
    PlotUtils::MnvH1D* bg_sub_data = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_bg_subbed_data->Clone("bg_sub_data");
    ...
    // Save to file
    std::string outfile_name = Form("ErrorSummary_BGSubData_%s_%s_%s_%s_%s", 
                                    p.m_variable->Name().c_str(), 
                                    p.m_do_frac_unc_str.c_str(),
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(),
  ...
  void PlotBGSub_ErrorSummary(EventSelectionPlotInfo p) {
    ...
    //PlotBGSub_ErrorGroup(p, "",                       0.0,  -1.);// // plot all groups together
    //PlotBGSub_ErrorGroup(p, "Flux",                   0.0,   0.1);//
    //PlotBGSub_ErrorGroup(p, "Detector",               0.0,   0.3);//
    ...
    Plot_ErrorGroup(p, bg_sub_data, "",                       "BGSub", 0.0,  -1.);// // plot all groups together
    ...
    Plot_ErrorGroup(p, bg_sub_data, "Genie_InteractionModel", "BGSub", 0.02,   0.2);//
...
//==============================================================================
  void Plot_Unfolded(EventSelectionPlotInfo p, MnvH1D* data, MnvH1D* mc,
    ...
    TH1D* effnum_true_w_stat_error = new TH1D(effnum_true->GetCVHistoWithStatError());
    ...
    // Draw
    const bool use_hist_titles = false;
    p.m_mnv_plotter.DrawDataMCWithErrorBand(unfolded_w_tot_error, effnum_true_w_stat_error, pot_scale, "TR", use_hist_titles);
    ...
    std::string outfile_name = Form("%s/Unfolded_%s_%s_%s%s%s", 
                                    outdir.c_str(),
                                    p.m_variable->Name().c_str(), 
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(), 
  ...
  void PlotUnfolded_ErrorSummary(EventSelectionPlotInfo p) {
    SetErrorGroups(p.m_mnv_plotter);
    PlotUtils::MnvH1D* unf = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_unfolded->Clone(uniq());
...
//==============================================================================
  void Plot_CrossSection(EventSelectionPlotInfo p, MnvH1D* data, MnvH1D* mc,
    ...
    if (do_bin_width_norm) {
      ...
      // Y label
      //std::string yaxis = "d#sigma/d" + p.m_variable->m_hists.m_xlabel + " (10^{-38} cm^{2}/" + p.m_variable->m_units + "/nucleon)";
      std::string yaxis = "d#sigma/d" + p.m_variable->m_hists.m_xlabel + " (10^{-42} cm^{2}/" + p.m_variable->m_units + "/nucleon)";
    ...
    for (int i = 0; i <= data_xsec_w_tot_error->GetNbinsX(); ++i ){
      ...
      std::cout << i << "  " << low_edge << "  " << up_edge << "  " << val << "  " << err << "  " << frac_err << "\n";
    }
    */
    ...
    // Draw
    const bool use_hist_titles = false;
    p.m_mnv_plotter.DrawDataMCWithErrorBand(data_xsec_w_tot_error, mc_xsec_w_stat_error, pot_scale, "TR", use_hist_titles);

    // Add chi2 label
    {
      const bool use_data_error_mtx = true;
      const bool use_only_shape_errors = false;
      const bool use_model_stat = false; // model statistical errors -- these are small if you use a priori effden, but atm I'm not. So keep this off.
      //p.m_mnv_plotter.AddChi2Label(data_xsec, mc_xsec, pot_scale, "TR", 0.03, -0.175, use_data_error_mtx, use_only_shape_errors); // this auto turns on model stat errors, I've manually turned it off within the function.

      //std::cout << "use_overflow is " << p.m_mnv_plotter.chi2_use_overflow_err  << "\n";
      // Check chi2
      int ndf = -1;
      //double pot_scale = 1.;
      double chi2 = p.m_mnv_plotter.Chi2DataMC(data_xsec, mc_xsec, ndf, pot_scale, use_data_error_mtx, use_only_shape_errors, use_model_stat);
    ...
    std::string outfile_name = Form("%s/CrossSection_%s_%s_%s%s%s", 
                                    outdir.c_str(),
                                    p.m_variable->Name().c_str(), 
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(), 
  ...
  void PlotCrossSection_ErrorSummary(EventSelectionPlotInfo p) {
    SetErrorGroups(p.m_mnv_plotter);
    PlotUtils::MnvH1D* xsec = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_cross_section->Clone(uniq());
    ...
    if(name=="enu") {
      detector_ymax = 0.2;
      FSI_threshold = 0.0085;      FSI_ymax = 0.045;
      Int_ymax = 0.08;
    }
    else if (name == "pmu") {
      detector_ymax = 0.2;
      FSI_threshold = 0.011;      FSI_ymax = 0.045;
      Int_ymax = 0.08;
    }
    else if (name == "ptmu") {
      detector_ymax = 0.2;
      FSI_threshold = 0.015;      FSI_ymax = 0.045;
      Int_threshold = 0.025;     Int_ymax = 0.2;
    }
    else if (name == "pzmu") {
      detector_ymax = 0.2;
      FSI_threshold = 0.015;      FSI_ymax = 0.045;
      ...
      detector_ymax = 0.25;
      FSI_threshold = 0.015;      FSI_ymax = 0.065;
      Int_threshold = 0.025;      Int_ymax = 0.3;
    }
    else if (name == "thetamu_deg") {
      detector_ymax = 0.25;
      FSI_threshold = 0.015;      FSI_ymax = 0.065;
      Int_threshold = 0.025;      Int_ymax = 0.15;
    }
    else if (name == "thetapi_deg") {
      FSI_threshold = 0.015;      FSI_ymax = 0.065;
      Int_threshold = 0.02;       Int_ymax = 0.12;
    }
    else if (name == "tpi") {
      detector_ymax = 0.3;
      FSI_threshold = 0.01;      FSI_ymax = 0.085;
      Int_ymax = 0.12;
    }
    else if (name == "wexp") {
      detector_ymax = 0.6;
      FSI_threshold = 0.008;      FSI_ymax = 0.06;
      Int_ymax = 0.1;
    }
    ...
    Plot_ErrorGroup(p, xsec, "Genie_InteractionModel", "CrossSection", 4e-3, 0.06);
  ...
  void PrintChi2Info(EventSelectionPlotInfo p, MnvH1D* data, MnvH1D* mc) {
    ...
    double chi2 = p.m_mnv_plotter.Chi2DataMC(data_xsec, mc_xsec, ndf, pot_scale, use_data_error_mtx, use_only_shape_errors, use_model_stat);
    use_model_stat = true;
    double chi2_mcstat = p.m_mnv_plotter.Chi2DataMC(data_xsec, mc_xsec, ndf, pot_scale, use_data_error_mtx, use_only_shape_errors, use_model_stat);
    //std::cout << p.m_variable->Name() << "\n";
    //std::cout << "   chi2 = "         << chi2     << "\n";
    //std::cout << "   ndf = "          << ndf      << "\n";
    //"   chi2/ndf = "     <<
    std::cout << p.m_variable->Name() << "  " <<  chi2/ndf << "  " << chi2_mcstat/ndf << "\n";
  }
...
//==============================================================================
  void PlotWSidebandFit_ErrorGroup(EventSelectionPlotInfo p, 
    ...
    std::string outfile_name = Form("ErrorSummary_%s_%s_%s_%s_%s", 
                                    tag.c_str(), 
                                    p.m_do_frac_unc_str.c_str(),
                                    p.m_do_cov_area_norm_str.c_str(), 
                                    GetSignalFileTag(p.m_signal_definition).c_str(),
  ...
  void PlotWSidebandFit_ErrorSummary(EventSelectionPlotInfo p, PlotUtils::MnvH1D* hist, std::string tag) {
    SetErrorGroups(p.m_mnv_plotter);
...
//==============================================================================
  ...
  void PlotBG_ErrorGroup(EventSelectionPlotInfo p, std::string error_group_name,
    ...
    PlotUtils::MnvH1D* tuned_bg = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_tuned_bg->Clone("tuned_bg");
    PlotUtils::MnvH1D* bg       = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_bg.hist->Clone("bg");
  ...
  */
  ...
  void PlotBG_ErrorSummary(EventSelectionPlotInfo p, bool do_tuned = false) {
    ...
    if (do_tuned)
      bg = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_tuned_bg->Clone("tuned_bg");
    else
    ...
    std::string name = p.m_variable->Name();
    if(name=="enu") {
      Int_threshold = 0.01;      Int_ymax = 0.12;
    }
    else if (name == "pmu") {
      Int_threshold = 0.01;      Int_ymax = 0.12;
    }
    else if (name == "ptmu") {
      Int_threshold = 0.01;     Int_ymax = 0.3;
    }
    else if (name == "pzmu") {
      Int_threshold = 0.01;      Int_ymax = 0.1;
    }
    else if (name == "q2") {
      Int_threshold = 0.01;      Int_ymax = 0.2;
    }
    else if (name == "thetamu_deg") {
      Int_threshold = 0.01;      Int_ymax = 0.15;
    }
    else if (name == "thetapi_deg") {
      Int_threshold = 0.01;       Int_ymax = 0.12;
    }
    ...
    Plot_ErrorGroup(p, bg, "Detector",               tuned_str, detector_threshold, detector_ymax);
    Plot_ErrorGroup(p, bg, "Flux",                   tuned_str, 0.0, 0.15);
    Plot_ErrorGroup(p, bg, "Genie_FSI_pions",        tuned_str, FSI_threshold, FSI_ymax);
    Plot_ErrorGroup(p, bg, "Genie_FSI_nucleons",     tuned_str, FSI_threshold, FSI_ymax);
    Plot_ErrorGroup(p, bg, "Genie_InteractionModel", tuned_str, Int_threshold, Int_ymax);
  ...
  void PlotBG_ErrorSummary(EventSelectionPlotInfo p, bool do_tuned = false) {
    ...
    //name, ignore threshold, ymax
    PlotBG_ErrorGroup(p, "",                       do_tuned, 0.0,  0.6); // plot all groups together
    PlotBG_ErrorGroup(p, "Flux",                   do_tuned, 0.0,  0.2);
    PlotBG_ErrorGroup(p, "Detector",               do_tuned, 0.0,  0.6);
...
// Hack-y functions
//==============================================================================
  void PlotTotalError(PlotUtils::MnvH1D* hist, std::string method_str, EventSelectionPlotInfo p ){
    TCanvas cF ("c4","c4"); 
    TH1D *hTotalErr = (TH1D*)hist->GetTotalError( p.m_include_stat, p.m_do_frac_unc, p.m_do_cov_area_norm ).Clone( Form("h_total_err_errSum_%d", __LINE__) );
    hTotalErr->SetTitle(Form("Total Uncertainty (%s)", method_str.c_str()));
    if (p.m_do_frac_unc)
      hTotalErr->GetYaxis()->SetRangeUser(0,1);
    //hTotalErr->Scale(0.408);
    hTotalErr->Draw();
    cF.Print(Form("TotalUncertainty_%s_%s_%s.png", p.m_do_frac_unc_str.c_str(), p.m_do_cov_area_norm_str.c_str(), method_str.c_str()));
  }


  void PlotStatError(PlotUtils::MnvH1D* hist, EventSelectionPlotInfo p, std::string tag, double ymax = -1., std::string ylabel = "") {
    TCanvas canvas ("c1","c1"); 
  ...
  void PlotVertBand(std::string band, std::string method_str, PlotUtils::MnvH1D* hist, EventSelectionPlotInfo p){
    TCanvas cF ("c4","c4"); 
    TH1* h1 = (TH1*)hist->GetVertErrorBand(band.c_str())->GetErrorBand(p.m_do_frac_unc, p.m_do_cov_area_norm).Clone(Form("Pmu_%s_%s", band.c_str(), method_str.c_str()));
  ...
  void PlotVertBandAllUniverses(std::string band, std::string method_str, PlotUtils::MnvH1D* hist, EventSelectionPlotInfo p){
    TCanvas cF ("c4","c4"); 
    p.SetTitle();
    p.SetXLabel(hist);
    hist->GetVertErrorBand(band.c_str())->DrawAll("",true);
    cF.Print(Form("%s_band_all_universes_%s.png", band.c_str(), method_str.c_str()));
  }
  ...
  void PlotDataMC(PlotUtils::MnvH1D* mc, PlotUtils::MnvH1D* data, EventSelectionPlotInfo p, std::string tag){
    TCanvas canvas ("c1","c1"); 
    double pot_scale = p.m_data_pot/p.m_mc_pot;
  ...
  void PlotDataMCWithError(PlotUtils::MnvH1D* mc, PlotUtils::MnvH1D* data, EventSelectionPlotInfo p, std::string tag) {
    std::cout << "Plotting\n";
    TCanvas canvas ("c1","c1"); 
    double pot_scale = p.m_data_pot/p.m_mc_pot;
  ...
  void PlotMC(PlotUtils::MnvH1D* hist, EventSelectionPlotInfo p, std::string tag, double ymax = -1., std::string ylabel = "") {
    TCanvas canvas ("c1","c1"); 
    double pot_scale = p.m_data_pot/p.m_mc_pot;
    p.SetXLabel(hist);
    // Y-axis range
    ...
    if (ylabel != "") hist->GetYaxis()->SetTitle(ylabel.c_str());
    //PlotUtils::MnvH1D* tmp_bg = nullptr;
    p.m_mnv_plotter.DrawMCWithErrorBand(hist); //I think that this call only shows stat errors.
    p.m_mnv_plotter.MultiPrint(&canvas, tag.c_str(), "png");
  }


  void PlotRatio(PlotUtils::MnvH1D* num, PlotUtils::MnvH1D* denom, const char* label, bool fixRange=true) {
...
//==============================================================================
  ...
  void PlotEfficiency_ErrorSummary(EventSelectionPlotInfo p) {
    SetErrorGroups(p.m_mnv_plotter);
    PlotUtils::MnvH1D* eff = (PlotUtils::MnvH1D*)p.m_variable->m_hists.m_efficiency->Clone(uniq());
