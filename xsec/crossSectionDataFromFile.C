...
void LoopAndFillData(const CCPi::MacroUtil& util, std::vector<Variable*> variables) {
  ...
  for(Long64_t i_event=0; i_event < util.GetDataEntries(); ++i_event) {
    ...
    event.m_passes_cuts = PassesCuts(*util.m_data_universe,
                                     event.m_reco_pion_candidate_idxs, is_mc,
...
void SaveDataHistsToFile(TFile& fout, std::vector<Variable*> variables) {
  ...
  for (auto v : variables) {
    ...
    v->m_hists.m_selection_data->GetXaxis()->SetTitle(v->m_hists.m_selection_mc.hist->GetXaxis()->GetTitle());
    v->m_hists.m_selection_data->Write(Form("selection_data_%s", name.c_str()));
    if (name == sidebands::kFitVarString) {
      v->m_hists.m_wsidebandfit_data->Write( Form("wsidebandfit_data_%s",
                                             name.c_str()) );
    }
  }
}
...
// reference.
void DoWSidebandTune(CCPi::MacroUtil& util, Variable* fit_var,
                    CVHW& loW_wgt, CVHW& midW_wgt, CVHW& hiW_wgt) {
  // DO FIT
    ...
    for(auto error_band : util.m_error_bands) {
      ...
      for(auto universe : universes) {
        WSidebandFitter wsb_fitter = WSidebandFitter(*universe, 
                                                     fit_var->m_hists,
                                                     util.m_pot_scale);
        wsb_fitter.Fit();
        // Store the outputs of the fits in HistWrappers
        loW_wgt.univHist(universe)->SetBinContent (1, (wsb_fitter.m_fit_scale)[kLoWParamId]);
        midW_wgt.univHist(universe)->SetBinContent(1, (wsb_fitter.m_fit_scale)[kMidWParamId]);
        hiW_wgt.univHist(universe)->SetBinContent (1, (wsb_fitter.m_fit_scale)[kHiWParamId]);
      }
    }
...
void RebinFitParamHists(UniverseMap error_bands, const double nbins,
         const CVHW& loW_wgt, const CVHW& midW_wgt, const CVHW& hiW_wgt,
         CVHW& loW_wgt_rebin, CVHW& midW_wgt_rebin, CVHW& hiW_wgt_rebin) {
  for(auto error_band : error_bands) {
    ...
    for(auto universe : universes) {
      ...
      for (int i = 1; i <= nbins; ++i) {
        //std::cout << loW_wgt_rebin.hist->GetXaxis()->GetBinLowEdge(i) << std::endl;
...
// return tuned BG
void ScaleBG(Variable* var, CCPi::MacroUtil& util,
            const CVHW& loW_wgt, const CVHW& midW_wgt, const CVHW& hiW_wgt) {
  // REBIN FIT PARAMS FOR THIS VARIABLE
    // temp HW's with same binning and error bands as variable
    PlotUtils::HistWrapper<CVUniverse> loW_wgt_rebin, midW_wgt_rebin, hiW_wgt_rebin;

    InitializeHW(var, Form("loW_fit_wgt_%s", var->Name().c_str()),
                 Form("W Sideband Fit Weight -- low W -- %s", var->Name().c_str()), 
                 util.m_error_bands, loW_wgt_rebin);
    InitializeHW(var, Form("midW_fit_wgt_%s", var->Name().c_str()),
    ...
    // For these MnvH1Ds ^ set each bin of each universe with the universe's fit weights
    RebinFitParamHists(util.m_error_bands, var->NBins(), 
                       loW_wgt,       midW_wgt,       hiW_wgt,
                       loW_wgt_rebin, midW_wgt_rebin, hiW_wgt_rebin);

  // APPLY TUNE TO EACH BG COMPONENT
    // tuned bg component = clone (untuned component)
    PlotUtils::MnvH1D* tuned_bg_loW  = (PlotUtils::MnvH1D*)var->m_hists.m_bg_loW.hist ->Clone(uniq());
    PlotUtils::MnvH1D* tuned_bg_midW = (PlotUtils::MnvH1D*)var->m_hists.m_bg_midW.hist->Clone(uniq());
    PlotUtils::MnvH1D* tuned_bg_hiW  = (PlotUtils::MnvH1D*)var->m_hists.m_bg_hiW.hist ->Clone(uniq());
  ...
  // WRITE TUNED BG
    ...
    tuned_bg->Write(Form("tuned_bg_%s", var->Name().c_str()));
      ...
      //  tuned_bg_loW ->Write(Form("tuned_bg_loW_POTscaled_%s",  var->Name().c_str()));
      //  tuned_bg_midW->Write(Form("tuned_bg_midW_POTscaled_%s", var->Name().c_str()));
      //  tuned_bg_hiW ->Write(Form("tuned_bg_hiW_POTscaled_%s",  var->Name().c_str()));
      //  tuned_bg     ->Write(Form("tuned_bg_POTscaled_%s", var->Name().c_str()));
...
void crossSectionDataFromFile(int signal_definition_int = 0,
  ...
  for (auto v : variables) {
    ...
    v->m_hists.m_selection_data->AddMissingErrorBandsAndFillWithCV(*v->m_hists.m_selection_mc.hist);
  }
  ...
  for (auto var : variables) {
    ...
    //============================================================================
      ...
      PlotUtils::MnvH2D* migration   = (PlotUtils::MnvH2D*)var->m_hists.m_migration.hist->Clone(uniq());
      PlotUtils::MnvH1D* bg_sub_data = (PlotUtils::MnvH1D*)var->m_hists.m_bg_subbed_data->Clone(uniq());
      int n_iterations = 4;
      if (var->Name() == "tpi" || var->Name() == "wexp" || var->Name() == "thetapi") n_iterations = 10;

      mnv_unfold.UnfoldHisto(var->m_hists.m_unfolded, migration, bg_sub_data, RooUnfold::kBayes, n_iterations);
      ...
      TH1D* unfolded_dummy    = new TH1D(var->m_hists.m_unfolded->GetCVHistoWithStatError());
      TH2D* migration_dummy   = new TH2D(migration->GetCVHistoWithStatError());
      TH1D* reco_dummy        = new TH1D(migration->ProjectionX()->GetCVHistoWithStatError());
      TH1D* truth_dummy       = new TH1D(migration->ProjectionY()->GetCVHistoWithStatError());
      ...
      var->m_hists.m_unfolded->PushCovMatrix(Form("unfolding_cov_matrix_%s",name), unfolding_cov_matrix_orig);
    ...
    /*
      ...
      { // Somehow effnum and effden have 200 flux universes
        MnvVertErrorBand *poppedFluxErrorBand = true_var->m_hists.m_effnum.hist->PopVertErrorBand("Flux");
      ...
      { // Somehow effnum and effden have 200 flux universes
        MnvVertErrorBand *poppedFluxErrorBand = true_var->m_hists.m_effden.hist->PopVertErrorBand("Flux");
        std::vector<TH1D*> fluxUniverses = poppedFluxErrorBand->GetHists();
        fluxUniverses.resize(100);
    ...
    */
      ...
      //h_efficiency_corrected_data->ClearSysErrorMatrices(); // maybe we'll write a new matrix when we divide? NOPE doesn't work.

      // Efficiency correct
      h_efficiency_corrected_data->Divide(var->m_hists.m_unfolded, var->m_hists.m_efficiency);

      TMatrixD unfolding_cov_matrix_effcor = 
          h_efficiency_corrected_data->GetSysErrorMatrix(Form("unfolding_cov_matrix_%s",name));
        ...
        { // Check to make sure the covariance matrix got divided correctly
          ...
          //    std::cout << unfolding_cov_matrix_effcor[j][i] - unfolding_cov_matrix_orig[j][i];
          //  }
          //  std::cout  << "\n";
          //}
        }
      ...
      h_efficiency_corrected_data->Write(Form("efficiency_corrected_data_%s", name));

    std::cout << "  Done Efficiency Correcting\n";


    //============================================================================
    // Normalization -- integrated flux, targets, POT (and don't forget MC, too!)
    //============================================================================
      ...
      static PlotUtils::FluxReweighter* frw = 
          new PlotUtils::FluxReweighter( 14, CCNuPionIncConsts::kUseNueConstraint, 
      ...
      //{ // Truncate flux universes to 10!!
          //  MnvVertErrorBand *poppedFluxErrorBand = h_flux_normalization->PopVertErrorBand("Flux");
          //  std::vector<TH1D*> fluxUniverses = poppedFluxErrorBand->GetHists();
      ...
      { //// Truncate flux universes to 10!!
        //MnvVertErrorBand *poppedFluxErrorBand = h_flux_normalization->PopVertErrorBand("Flux");
        //std::vector<TH1D*> fluxUniverses = poppedFluxErrorBand->GetHists();
        //std::cout << "flux universes: " << fluxUniverses.size() << "\n";
        ////fluxUniverses.resize(10);
        ////h_flux_normalization->AddVertErrorBand("Flux",fluxUniverses);
      }
      ...
      // back on
      TMatrixD unfolding_cov_matrix = h_cross_section->GetSysErrorMatrix(Form("unfolding_cov_matrix_%s",name));
      for(int i=0; i<unfolding_cov_matrix.GetNrows(); ++i) unfolding_cov_matrix(i,i)=0;
      h_cross_section->PushCovMatrix(Form("unfolding_cov_matrix_%s",name), unfolding_cov_matrix);
...
//PlotUtils::MnvH1D* efficiency_numerator   = (PlotUtils::MnvH1D*)true_var->m_hists.m_effnum.hist->Clone(uniq());
//PlotUtils::MnvH1D* efficiency_denominator = (PlotUtils::MnvH1D*)true_var->m_hists.m_effden.hist->Clone(uniq());
