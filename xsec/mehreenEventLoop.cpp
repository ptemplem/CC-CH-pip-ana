#define MC_OUT_FILE_NAME "runEventLoopMC_Dec312021_q3bin2_atleast1pi_distcuts.root"
#define DATA_OUT_FILE_NAME "runEventLoopData_Dec312021_q3bin2_atleast1pi_distcuts.root"
 	...
 	for (int i = 0; i<nbins; i++){
        	if (tpi <= tpibin[0]) weight2 = weight*currentbins[0];
        		...
        		if (tpi > binedge1 && tpi < binedge2 && tpi < 0.5) {
            			weight2 = weight*currentbins[i];
        		}
        		}
        		else{
          	  		if (tpi > .500 ) weight2 = weight*currentbins[22];
        		}
    	}
i*/

        for(auto& var: vars) var->selectedMCReco->FillUniverse(universe, var->GetRecoValue(*universe), weight2); //"Fake data" for closure
        ...
        else
        {
          ...
          for(auto& var: vars) (*var->m_backgroundHists)[bkgd_ID].FillUniverse(universe, var->GetRecoValue(*universe), weight2);
...
int main(const int argc, const char** argv)
{
  ...
  TFile* mc_MichelStudies = TFile::Open("Dec312021_noreweight_distcuts_q3bin2_atleast1pi_MC.root", "RECREATE");
  TFile* data_MichelStudies = TFile::Open("Dec312021_noreweight_distcuts_q3bin2_atleast1pi_data.root", "RECREATE");
  ...
  std::vector<double> dansPTBins = {0, 0.075, 0.15, 0.25, 0.325, 0.4, 0.475, 0.55, 0.7, 0.85, 1, 1.25, 1.5},
...
std::function<double(const CVUniverse&, const MichelEvent&, const int)> true_angle_range4 = [](const CVUniverse& univ, const MichelEvent& evt, const int whichMichel)
   ...
   studies.push_back(new PerMichelVarByGENIELabel(permichel_tpi, "Per_Michel_PrimaryParentKE", "meV", 100, 0, 1000., error_bands));
  ...
  // studies.push_back(new PerMichel2DVar(overlay_vtx_tpi, overlay_vtx_range, tpi_config, pirange_config, error_bands));
  // studies.push_back(new PerMichel2DVar(overlay_clus_tpi, overlay_clus_range, tpi_config, pirange_config, error_bands)); 
  // studies.push_back(new PerMichel2DVar(truemichel_goodvtx_range, truemichel_goodvtx_tpi, tpi_config, pirange_config, error_bands));
  // studies.push_back(new PerMichel2DVar(truemichel_goodclus_range, truemichel_goodclus_tpi, tpi_config, pirange_config, error_bands));
  // studies.push_back(new PerMichel2DVar(truemichel_badvtx_range, truemichel_badvtx_tpi, tpi_config, pirange_config, error_bands));
  // studies.push_back(new PerMichel2DVar(truemichel_badclus_range, truemichel_badclus_tpi, tpi_config, pirange_config, error_bands));
...
std::function<double(const CVUniverse&, const MichelEvent&)> lowesttpi = [](const CVUniverse& univ, const MichelEvent& evt)
{
   ...
   studies.push_back(new PerMichelEventVarByGENIELabel(lowesttpi, "LowestKE_pion", "MeV", 100, 0, 1000., error_bands));
